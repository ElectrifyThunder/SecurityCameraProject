...
from tkinter import Frame,Entry,Label,Checkbutton,Button #Confirmed Labels I'll be using today.

accountsFileName = "savedAccounts.txt"

class MainRoot(): #MainRoot Window
    def __init__(self, cameraNumber,cameraAmount):
...
class loginRoot():  #Get the login cridentials
    #Username, Password, NewAccount Boolean Here.
...
def switchMainCamera(prisonerNumber:Label, prisonerLabel:Label,camFrame:Frame,mainframe:Frame):
    #This func changes the main camera labels before updating the main camera
...
def loadMainProgram():
    ...
    for i in range(AmountOfCameras): 
        ...
        goToCamera = Button(
            ...
            font=("Terminator", 12),
            fg="White",
            command=lambda frame=newFrame: switchMainCamera(cameraNumber, prisonerName,frame,mainFrame)
        )
        goToCamera.pack(expand=True)

        #Update prisoner name labels
...
#Content Box
outerYellowFrame = Frame(root, bg="yellow", width=1200, height=600) 
outerYellowFrame.pack_propagate(False)
outerYellowFrame.place(relx=0.5, rely=0.5, anchor="center")

contentFrame = Frame(outerYellowFrame, bg=root["bg"])
contentFrame.pack(expand=True, fill="both", padx=3, pady=3) 
